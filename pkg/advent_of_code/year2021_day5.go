...
func Year2021Day5Part2(cache bool) string {
  content := aoc_utils.DownloadInput(2021, 5, cache)
  input := aoc_utils.InputToSlice(content)
  coordinateLines := [][]int{}
  for _, row := range input {
  ...
  knownPoints := make(map[string]int)
  for _, endpoints := range coordinateLines {
    knownPoints = Y21D5TallyLinePoints(knownPoints, endpoints, true)
  }
...
func Y21D5ExtractEndpoints(line string) []int {
  result := []int{}
...
func Y21D5LinePoints(endpoints []int, includeDiagonal bool) []string {
  result := []string{}
  min := 0
  max := 0
...
func Y21D5TallyLinePoints(knownPoints map[string]int, endpoints []int, includeDiagonal bool) map[string]int {
  linePoints := Y21D5LinePoints(endpoints, includeDiagonal)
