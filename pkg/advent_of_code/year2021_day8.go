...
func Year2021Day8Part1(cache bool) string {
...
func Year2021Day8Part2(cache bool) string {
  content := aoc_utils.DownloadInput(2021, 8, cache)
  input := aoc_utils.InputToSlice(content)
  res := 0
  for _, row := range input {
    rowVal :=  Y21D8DecodeRow(row)
    if rowVal == -1 {
...
func Y21D8DecodeRow(row string) int {
  ...
  result, digitMap := Y21D8DecodeOutput(output, digitMap)
  if result > 0 {
    return result
  }
  ...
  for i := 0; i < 3; i++ {
    for _, digit := range strings.Split(input, " ") {
      decodedDigit := Y21D8DecodeValue(digit, digitMap)
      if decodedDigit != "UNKNOWN" {
        digitMap[decodedDigit] = digit
        result, newDigitMap := Y21D8DecodeOutput(output, digitMap)
...
func Y21D8DecodeOutput(output string, digitMap map[string]string) (int, map[string]string) {
  decodedVals := 0
  outputVal := ""
  newDigitMap := digitMap
  for _, digitChars := range strings.Split(output, " ") {
    digitVal := Y21D8DecodeValue(digitChars, digitMap)
    if digitVal != "UNKNOWN" {
...
func Y21D8DecodeValue(digit string, digitMap map[string]string) string {
